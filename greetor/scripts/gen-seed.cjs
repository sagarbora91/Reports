/* Generate ~6 months of schema-accurate dummy data for Saagar Greetor.
   Built THROUGH the app's own data layers so the seed can never drift from
   the real schema. Emits greetor/www/seed-data.js (window.DEMO_SEED).
...
const greetors = [g1, g2, g3, manager];

const stores = Masters.storeNames(state);
...
const mobilePool = [];
for (let i = 0; i < 700; i++) mobilePool.push(String(int(6, 9)) + String(int(100000000, 999999999)));
...
const banner = "// AUTO-GENERATED demo data (~6 months) for testing. Do NOT hand-edit.\n"
  + "// Loaded on a fresh install only. Demo logins — Owner 1234 · Manager 2345 · Greetors 3456/4567/5678\n";
fs.writeFileSync(path.join(WWW, "seed-data.js"), banner + "window.DEMO_SEED = " + JSON.stringify(state) + ";\n");
const bytes = Buffer.byteLength(JSON.stringify(state), "utf8");
console.log("seed-data.js written. records=" + recCount + " conv=" + convCount + " withMobile=" + withMobile
  + " repeats=" + repeats + " quick=" + quickCount + " audit=" + state.auditLog.length
  + " comms=" + state.commsLog.length + " footfall=" + Object.keys(state.footfall.entries).length
  + " blob=" + (bytes / 1024 / 1024).toFixed(2) + "MB");
